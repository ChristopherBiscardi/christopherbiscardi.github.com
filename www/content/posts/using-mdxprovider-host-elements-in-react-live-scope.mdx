---
title: Using MDXProvider host elements in react-live scope
date: 2019-08-18
tags:
  - gatsby
  - mdx
...
Now we've established that [`useMDXScope` grabs from the context for imports](/post/using-mdx-scope-in-react-live-scope/) and [`useMDXComponents` grabs from the combined value of `MDXProvider`s](/post/using-mdx-shortcodes-in-react-live-scope/). There is one more missing piece: using host-level elements defined in `MDXProvider`. An example of using `MDXProvider` to override the rendering of all links in an MDX document:
