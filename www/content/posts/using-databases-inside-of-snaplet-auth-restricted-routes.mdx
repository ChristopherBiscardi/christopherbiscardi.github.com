...
date: 2014-01-09
slug: "/2014/1/9/using-databases-inside-of-snaplet-auth-restricted-routes/"
---
...
The app we’re using was built by running `snap init` and
...
In `src/Site.hs` we’ve added the following language
extensions:
...
We've also defined a convenience function `needsAuth` to
restrict our `"/postgres"` route to only logged in users.
...
What's really doing the heavy lifting of using the postgres
snaplet inside of the auth snaplet's route is this instance:
...
> -- | Like 'with' but doesn't impose the requirement that
> the action -- being run be a descendant of the current
> snaplet.
...
to ask the parent context (using `withTop`).
