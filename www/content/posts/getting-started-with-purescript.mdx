...
date: 2014-06-22
slug: "/2014/6/22/getting-started-with-purescript/"
---

According to
[PureScript.org](http://www.purescript.org/ "purescript.org")

> PureScript is a small strongly, statically typed
> programming language with expressive types, written in and
> inspired by Haskell, and compiling to Javascript.

It’s fairly easy to get started with the
[grunt-init-purescript template](https://github.com/purescript-contrib/grunt-init-purescript "grunt
...
and [bower](http://bower.io/) to work with this PureScript
template. [Node](http://nodejs.org/ "nodejs") is also a
dependency because of this.

```bash
npm install -g grunt grunt-init bower
```

Then clone the template into a `.grunt-init` folder using
git:
...
We also need the PureScript compiler, which we can grab with
cabal:
...
`purescript-quickcheck`, `either` and `maybe` to name a few.
...
such, our `Main` module is in `tests/Test.purs` and can be
run using:
...
To build an executable (a node.js scraper for example) we
...
and change the module name from `Main` to `Test.Main`. Also
change the `Starter.Kit.Example` import to
`Starter.Kit.Main`.
...
Now we will make a small modification to the Starter Kit
file.
