...
Run `cilium monitor` with the `--type=drop` filter to see
dropped packets. Run it inside a pod (`cilium-8nslj`) that
exists in a namespace (`cilium`).
...
list cilium endpoints, their Identity, and their enforcement
state for ingress and egress.
