...
`createTypes` allows the introduction or "polishing" of
types.

you can use directives to force inference or non-inference
of fields.
...
multiple interfaces for BlogPostWithComments,
BlogPostWithMultipleAuthors, BlogPostv2, etc, all available
so that themes can choose which one they support

define new interface (BlogPostWithComments), old resolvers
are already defined, so grab originalResolver off the type
and execute.
...
can do this, but it's a stop-gap and will be more formally
supported
...
Replaces `setFieldsOnGraphQLNodeType()` (but the API isn't
deprecated yet).
