export const meta = {
  ...
  slug: "/2014/2/2/deploy-haskells-snap-on-heroku/"
};
...
First, we need to put a `Procfile` in the root of our
project; Save this as `Procfile`
...
`git init`, `git add` and `git commit -m "message"`.

This command will create a new Heroku app with a Haskell
buildpack. You can find more information on the buildpack
...
> note: You can now deploy the app to Heroku, but you will
> get an error about connecting to PostgreSQL
...
The final option is to just host PostgreSQL somewhere other
than Heroku and modify the config file to point there.
