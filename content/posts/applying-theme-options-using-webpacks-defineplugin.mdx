...
Wherever we use this flag, webpack will replace the value
with a boolean literal, so we can use this to enable or
disable features of our application.
...
consider using a
[custom configuration node](/post/applying-theme-options-using-custom-configuration-nodes/)
instead, as that will let you `useStaticQuery` to access
