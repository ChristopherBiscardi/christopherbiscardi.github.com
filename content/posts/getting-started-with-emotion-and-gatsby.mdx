export const meta = {
  ...
  tags: ["css-in-js", "javascript", "gatsby", "emotion"],
  egghead:
    "https://egghead.io/lessons/gatsby-getting-started-with-emotion-and-gatsby/embed",
  slug: "/post/getting-started-with-emotion-and-gatsby"
};

[Emotion](https://emotion.sh/) is a library for authoring
and composing CSS rulesets in a performant way. Here's how
to get started using it with
[Gatsby](https://www.gatsbyjs.org/).

First, we'll boot up a fresh gatsby site using the default
starter and install the Gatsby plugin (as well as Emotion
itself).
...
Then we need to add `gatsby-plugin-emotion` to our
`gatsby-config.js`. We'll add it at the top of our plugins
list.
...
We can run `yarn develop` to run our new Gatsby site. The
...
at `src/pages/index.js`, we see the default starter uses
inline styles.
...
The result of this page (specifically the `div` containing
...
the output and focus on the root `div` with the inline
styles. We can see that they compile to a `style` attribute.
...
change we need to make in this file is to swap `styles` for
`css`.
...
Well in `<head>` there's a new `<style>` tag that includes
