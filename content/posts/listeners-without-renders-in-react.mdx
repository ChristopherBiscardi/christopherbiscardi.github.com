export const meta = {
  ...
  slug: "/post/listeners-without-renders-in-react"
};
...
handle this by rendering `null` and using lifecycle events
to deal with listener attachment and cleanup.

The following code creates two components, one representing
the App and one to handle the logic. `App` mounts or
unmounts the `KeydownListener` once every second, forcing
`componentDidMount` and `componentWillUnmount` to fire.
...
class App extends Component {
  ...
  render() {
    return (
      <div>{this.state.show && <KeydownListener />}</div>
    );
  }
}
...
The output for the above is the following (which you can
test in [codesandbox](https://codesandbox.io/s/yjz3lpmklx).
