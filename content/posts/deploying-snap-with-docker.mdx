export const meta = {
  ...
  slug: "/2014/10/15/deploying-snap-with-docker/"
};
...
In our previous post we built out a scaffold project with
Snap 1.0. In this post we’ll go over building a simple
Dockerfile to deploy the project.

Make sure you’ve
[installed docker](https://docs.docker.com/installation/#installation)

## Building the Haskell Base

There is an effort to make a
[docker official haskell image](https://github.com/darinmorrison/docker-haskell/tree/docker-library)
so we’ll use that. Once it’s merged into `docker-library`,
we can simply skip building the `docker-haskell` from
scratch.

Clone docker-haskell and switch to the docker-library
branch. (It doesn’t matter where on the computer we do this)
...
`-t` is short for `--tag`, which lets us tag an image. We’ll
use `haskell` for the base name and tag it as `7.8`.
Finally, the `.` tells docker where to find the dockerfile
we’re building.
...
[`docker run`](https://docs.docker.com/reference/commandline/cli/#run)
to spawn `ghci` as an interactive pseudo-tty.
...
> NOTE: We can push this base image
> [to the docker hub](https://docs.docker.com/userguide/dockerrepos/#pushing-a-repository-to-docker-hub)
> but interestingly enough, we don’t have to (if we’re using
> “normal” repos; Automated Builds are a different story).
> We can push the final image without pushing the base
> image.

### Dockerfile

Our `Dockerfile` goes in the root of the `auth-server/`
...
With the above dockerfile in the root of `auth-server/` we
can build with:
...
> NOTE: `-t auth-server` could be any name, such as
> `-t myawesomething` but if you plan to push it to the
> docker hub do `username/image-name:tag-name`
...
port `8000` (or at `boot2docker ip` on port `8000`).

### To The Hub!

We can push the image we just built to a registry (such as
the docker hub) by building it with our username (so it gets
filed under our user on the hub):
...
and on some other computer (such as AWS, a Digital Ocean
instance or another dev computer) pull and run the image:
