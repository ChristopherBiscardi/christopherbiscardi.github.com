export const meta = {
  ...
  tags: ["gatsby", "mdx"],
  slug: "/post/removing-gatsby-mdx-wrappers"
};

I just
[removed generated wrappers](https://github.com/ChristopherBiscardi/gatsby-mdx/pull/221)
from
[gatsby-mdx's](https://github.com/ChristopherBiscardi/gatsby-mdx)
implementation. This removed a sticking point for a lot of
people from the API surface: `componentWithMDXScope`. People
...
was accessible via context to the `MDXRenderer`. Wrappers
were only necessary if you wanted to use MDX content from a
remote CMS, like Contentful, or if you wanted to store MDX
files somewhere that wasn't `src/pages` and also wanted to
generate your own URLs, etc from them. Essentially, if you
ever used `createPage` to point to MDX content you probably
used Wrappers.

What happened was in a `gatsby-node` `createPage` call,
...
You would wrap the component in `componentWithMDXScope` (and
...
hoisted the user's `pageQuery` in their template to the
...
First of all, the `pageQuery` in the original template never
...
That's not the only confusion it caused either. The biggest
source of confusion was "why do we need
componentWithMDXScope... at all?", and plenty of people
forgot or didn't know to use it.

# Removing Wrappers

So, I was documenting `componentWithMDXScope` to explain
...
changed `componentWithMDXScope` to a no-op. This is so that
...
Then, taking advantage of Gatsby's `wrapRootElement` API in
`gatsby-browser.js` and `gatsby-ssr.js` I inserted all of
...
module.exports = () => {
  const files = fs.readdirSync(
    "./.cache/gatsby-mdx/mdx-scopes-dir"
  );
...
...and that's it. `componentWithMDXScope` is gone and users
