export const meta = {
  ...
  slug: "/post/styled-system-on-react-hooks"
};

I'm pretty confident that the next generation of
design-system tools like styled-system will be built on
React hooks. So let's take a look at the early stages of how
that might happen.

First, we'll init a default
...
[some sweet APIs for adding providers to the root of the application](/post/controlling-the-gatsby-application-root)
so we'll install `gatsby-plugin-emotion` to grab Emotion 10
and write a
...
We're using `emotion-theming` instead of a raw context for
the memoization, theme merging, and
[other features](https://github.com/emotion-js/emotion/blob/02ff1e2e8cacf760907a8c1bdbccdde48450ebe2/packages/emotion-theming/src/theme-provider.js)
...
Because we're going to use hooks, we need the React alphas
too.
...
So let's start taking out some of the app and moving values
into our theme.
...
with their theme equivalents. We also replace `style` with
...
[`Box` component from superbox](https://github.com/jxnblk/superbox/blob/70be4c76ff2695e6d6c7c6a3bc6c98991c611577/src/index.js).
...
Now `MyThing` supports the spacing scale, font scale, text
colors and more. Applicable to any element you're creating
and without a special `styled` API so it becomes "just
...
If we have `useBox` return a css object, we can also compose
...
What's the best hooks API for styled-system? Not a clue!
We've only begun to explore the API design space, so I would
be not surprised at all to see many different types of
implementations arise over the next year as React hooks go
stable release.

h/t to [John](https://twitter.com/4lpine) and
[Brent](https://twitter.com/jxnblk) for some discussions
today that could prove very interesting in the future :)
