export const meta = {
  ...
  date: "2014-10-17",
  slug: "/2014/10/17/emacs-in-docker/"
};
...
docker) and a version of `1.3` or higher, we can use Volumes
...
set up a function to mount the `pwd` at `/files` inside the
...
and you’ll be in an emacs. All edits in `/files` will
persist to the host:
...
To build your own `docker-emacs`, check out my
[Automated Build](https://registry.hub.docker.com/u/biscarch/emacs/)
and the
[GitHub Repo](https://github.com/ChristopherBiscardi/docker-emacs)
for `biscarch/emacs`
