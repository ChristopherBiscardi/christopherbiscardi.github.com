export const meta = {
  ...
  slug: "/post/kafka-kube-and-statefulsets-on-gke"
};

# Kubernetes StatefulSets

> StatefulSet is the workload API object used to manage
> stateful applications. StatefulSets are beta in 1.8.
>
> [k8s docs](https://kubernetes.io/docs/concepts/workloads/controllers/statefulset/)

[Kafka][kafka] and [Zookeeper][zookeeper] are two of the
motivating examples for StatefulSets in [Kubernetes][k8s].
Being stateful applications, we'll need disks to store the
data on. GKE can help us allocate disks and compute for
brokers even as we scale the service up.
...
the node type to `n1-standard-2` which will be able to
...
We'll use the [Zookeeper][zookeeper-statefulset] and
[Kafka][kafka-statefulset] example configs to start. First,
...
will `Error`, then `CrashLoopBackOff` until it can connect
to Zookeeper.
...
We can use the following command to interactively choose a
kafka container to exec into.
...
down into the stateful node identifier (`zk-0`), the service
name (`zk-svc`), and the network/namespace defaults (as well
as the port).
...
then exec into the same container again and run the producer
so we can send messages to the consumer.
...
`StatefulSet`, `Service`, and `PodDisruptionBudget`.
Zookeeper also uses a `ConfigMap` instead of a file, etc

### Zookeeper

#### Service

The Zookeeper `Service` is the first object we see in the
yaml file.
...
This block says we're creating a `Service` with a name of
`zk-svc` (remember the URLs we had to use to access
Zookeeper earlier). We've removed the `clusterIP` which
makes this a [Headless Service][headless-service]. Since
...
Next we define a `ConfigMap`, which contains configuration
...
Next we create a `PodDisruptionBudget` that lets us ensure
that a minimum of 2 nodes will be available at any given
time _due to voluntary disruptions like upgrades_. It's
...
Ah yes, the central dish to our exploration, the
[StatefulSet][statefulsets].

We set the affinity for the Zookeeper pod to try to place
itself on a node that doesn't already have a Zookeeper node.
...
The volume mounts allocate a volume named `datadir` and
claim `10Gi` of storage. This is a generic way to "claim"
...
The Kafka yaml file has basically the same components so I
won't go over it here.

Congrats, you're running Kafka on GKE. This should be good
enough for any testing you'd want to run. In the next post
in this series we'll go over how to use the Confluent
Platform instead of the containers specified in these yaml
files.
        ...
        - name: k8skafka
          ...
          command:
            ...
            - "exec kafka-server-start.sh
              /opt/kafka/config/server.properties --override
              broker.id=${HOSTNAME##*-} \
...
[zookeeper-statefulset]:
  https://github.com/kubernetes/contrib/tree/0c2c22990617bc5ddb696ae7171ecbe1ca208d17/statefulsets/zookeeper
[kafka-statefulset]:
  https://github.com/kubernetes/contrib/tree/0c2c22990617bc5ddb696ae7171ecbe1ca208d17/statefulsets/kafka
[statefulsets]:
