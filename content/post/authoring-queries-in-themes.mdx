...
This is very flexible in that it allows us to access every field as we're used to on `Mdx` or any other type. It is restricting in that we can't combine multiple types to provide notes. To do that, we'll have to look in to [Constructing Query Types in Themes](/posts/constructing-query-types-in-themes).
