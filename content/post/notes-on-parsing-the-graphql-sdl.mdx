...
tags:
  ...
  - graphql
---
Lately I've been doing some work with the [GraphQL SDL](https://www.prisma.io/blog/graphql-sdl-schema-definition-language-6755bcb9ce51) and [graphql-js][graphql-js] (because it's for [Gatsby](https://www.gatsbyjs.org/) and Gatsby is JavaScript). This post is an attempt to demystify some of the code inside of graphql-js. We're not starting at the beginning so buckle up.
...
Since this is a custom language it means we need a way to represent it in JavaScript to do anything more than logging it out. Enter [graphql-js][graphql-js].

## parse

[graphql-js][graphql-js] is (TODO:). The package has a `language` module built in which is responsible for defining the [lexer][lexer] and [parser][parser] for the SDL. Gatsby [re-exports this package](https://github.com/gatsbyjs/gatsby/blob/4fc1d45bcfe19790c8a94267f2a8d1cd38891a95/packages/gatsby/graphql.js) with an additional JSON datatype so we have to make sure we only import one of the packages' `graphql`s because you can only have one graphql-js (*why* you can only have one version installed is a story for another day and is a limitation of `graphql-js` itself).
...
The less complex of the two is our `Post` type. We can see that it's an `Object` type with a`name` of `Post`. It has no `interfaces` and no `directives` and a single `Field`. On our `Field`, we get the name and return type of the field (`title` and `String`)
...
`visit` allows us to define a set of nodes to process. We can change them here too but in this case we'll just pretty-print them. Since we're clearly working with an [AST](https://en.wikipedia.org/wiki/Abstract_syntax_tree) now, we'll name our argument in the `forEach` `ast` which we expect to be each of the Object types we've defined.
...
If you want to explore the AST more without having to write your own code, I highly suggest [AST Explorer](https://astexplorer.net/#/gist/9bb73e8b0d49f11d0e91b21a1f52aadd/d0b34c79e824f29f7d9b1f1cee24256d3b6f484e), which is a browser-based tool for viewing ASTs. I've added the example in this blogpost to the AST Explorer link.
