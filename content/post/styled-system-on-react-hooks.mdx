...
```js
const Header = ({ siteTitle, color = "white", backgroundColor = "brand" }) => {
...
const MyThing = props => {
  ...
  return <section {...otherProps} css={boxProps} />
}
```
...
const MyThing = props => {
  const [cssProps, otherProps] = useBox(props);
  return <section {...otherProps} css={{ ...boxProps, lineHeight: 1.5 }} />;
};
```

So we can even get more specific like `useTextColor`.
