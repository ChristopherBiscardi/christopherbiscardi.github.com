...
This time we can see that the containing div has a classname on it instead of inline styles... but where are the styles? Well in `<head>` there's a new `<style>` tag that includes our styles and a source map (this is development after all).
