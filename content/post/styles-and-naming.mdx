...
Element Modifier (BEM)][bem], mirroring the hand-crafted approach our
...
example from [Bootstrap][bootstrap], a popular library that offers a
CSS API.
...
([CSSOM][cssom]) defines a set of parsing and serialization rules for
...
1. (private) sub-components
1. (public) sub-components (components that get too complex for a
   single file, thus are nested under in a directory. routes.)
1. A project's components/ folder ("Named objects local to this project")
...
[Emotion][emotion] is a library that introduces a new set of
...
// TODO: full pagination implementation and API rethink
