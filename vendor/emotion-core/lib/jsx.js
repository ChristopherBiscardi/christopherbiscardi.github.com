...
import { serializeStyles } from "@emotion/serialize"; // those identifiers come from error stacks, so they have to be valid JS identifiers
...
let render = (cache, props, theme, ref) => {
  let cssProp = theme === null ? props.css : props.css(theme); // so that using `css` from `emotion` and passing the result to the css prop works
...
let Emotion =
  /* #__PURE__ */
  withEmotionCache((props, cache, ref) => {
    // use Context.read for the theme when it's stable
    if (typeof props.css === "function") {
      return h(ThemeContext.Consumer, null, theme =>
        render(cache, props, theme, ref)
      );
    }
